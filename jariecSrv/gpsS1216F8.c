...
//gpsx:nmea信息结构体
//buf:接收到的GPS数据缓冲区首地址
int  NMEA_GNGGA_Analysis(nmea_msg *gpsx,u8 *buf)
{
	u8 *p1 ,dx;			 
	u8 posx;    
...
//gpsx:nmea信息结构体
//buf:接收到的GPS数据缓冲区首地址
int  NMEA_GNGSA_Analysis(nmea_msg *gpsx,u8 *buf)
{
	u8 *p1,dx;			 
	u8 posx; 
	u8 i;   
...
u8 SkyTra_Cfg_Prt(int fd, u32 baud_id, void *buffer)
{
	SkyTra_baudrate *cfg_prt=(SkyTra_baudrate *)buffer;
	...
	SkyTra_Send_Date(fd, (u8*)cfg_prt,sizeof(SkyTra_baudrate));//发送数据给SkyTra   
	usleep(200 * 1000);				//等待发送完成 
	return SkyTra_Cfg_Ack_Check();//这里不会反回0,因为UBLOX发回来的应答在串口重新初始化的时候已经被丢弃了.
} 
//配置SkyTra_GPS/北斗模块的时钟脉冲宽度
//width:脉冲宽度1~100000(us)
//返回值:0,发送成功;其他,发送失败.
u8 SkyTra_Cfg_Tp(int fd, u32 width, void *buffer)
{
	u32 temp=width;
	SkyTra_pps_width *cfg_tp =(SkyTra_pps_width *)buffer;
	...
	SkyTra_Send_Date(fd, (u8*)cfg_tp,sizeof(SkyTra_pps_width));//发送数据给NEO-6M  
	return SkyTra_Cfg_Ack_Check();
}
//配置SkyTraF8-BD的更新速率    
//Frep:（取值范围:1,2,4,5,8,10,20,25,40,50）测量时间间隔，单位为Hz，最大不能大于50Hz
//返回值:0,发送成功;其他,发送失败.
u8 SkyTra_Cfg_Rate(int fd, u8 Frep, void *buffer)
{
	...
	SkyTra_Send_Date(fd, (u8*)cfg_rate,sizeof(SkyTra_PosRate));//发送数据给NEO-6M 
	return SkyTra_Cfg_Ack_Check();
}
//发送一批数据给Ublox NEO-6M，这里通过串口3发送
//dbuf：数据缓存首地址
//len：要发送的字节数
void SkyTra_Send_Date(int fd, u8* dbuf,u16 len)
{
	write(fd, dbuf, len);
}
